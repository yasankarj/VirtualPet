...
| File | Purpose |
|---|---|
| `src/domain/types.ts` | `PetStats`, `ActionCooldowns`, `PetState`, `MoodState` |
| `src/domain/constants.ts` | All tunable constants (NFR4) — ticks, deltas, cooldowns, thresholds, storage key |
| `src/domain/factory.ts` | `createNewPet()` |
| `src/domain/rules.ts` | Pure functions: `clamp`, `computeHealth`, `computeMood`, `applyDecay`, `applyRestTick`, `applyCooldownCountdown`, `tick`, `applyFeed`, `applyPlay`, `applyRest` |
...
| File | Type | Covers |
|---|---|---|
| `tests/domain/rules.test.ts` | Example-based | Feed/Play/Rest effects and preconditions (no-op on cooldown/resting), cooldown countdown, Health decline/recovery, full Mood priority order |
